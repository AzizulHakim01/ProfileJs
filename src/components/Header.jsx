  ...
  return (
    <div className="bg-gradient-to-r from-teal-300 to-teal-700">
      <header className="flex justify-between w-[1152px] mx-auto h-[10vh] items-center">
        ...
        {user ? (
          <>
            <div className="flex gap-4 items-center cursor-pointer relative" onClick={()=> setShowMenu(!showMenu)}>
              <img
              ...
              <div className={`absolute top-8 left-16 bg-white px-4 py-2 rounded-md ${!showMenu && "hidden"} flex flex-col gap-4`}>
                <span className="font-semibold text-zinc-600">Dashboard</span>
                <span className="font-semibold text-zinc-600" onClick={handleLogOut}>Log out</span>
              </div>
            </div>
          </>
        ) : (
